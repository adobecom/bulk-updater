...
// import { mdast2docx } from '@adobecom/milo-helix-md2docx';
...
import DEFAULT_STYLES from './defaultstyles.js';

const mdast2docxOptions = { stylesXML: DEFAULT_STYLES };
...
async function getMergedMdast(langstoreNowProcessedMdast, livecopyProcessedMdast) {
      ...
      if (mergedArr[i].hashcode === contentHash) {
        ...
        if (mergedArr[i].classType === 'deleted') {
          if (type === 'added') {
            ...
            newArr.push({ hashcode: contentHash, classType: '' });
            return newArr;
          }
          if (type === 'deleted') {
            mergedArr.push({ hashcode: contentHash, classType: type });
            return mergedArr;
          }
        }
  ...
  for (index = 0; index < length; index += 1) {
    if (langstoreNowProcessedMdast[index] === livecopyProcessedMdast[index]) {
      ...
      if (langstoreContent?.type === 'list' && livecopyContent?.type === 'list'
        ...
        for (let j = 0; j < diffedListContent.children.length; j += 1) {
          ...
          if (child.action === 'deleted' && child.author === 'Langstore Version'
            ...
            console.log(diffedListItemContent);
        ...
        mergedProcessedMdast = checkAndPush(mergedProcessedMdast, mergedListHash, '');
      } else {
...
async function main() {
  const langstoreMd = await readFile('./langstore-list.md', 'utf8');
  const livecopyMd = await readFile('./regional-list.md', 'utf8');
  // const langstoreMd = await readFile('./langstore-text.md', 'utf8');
  // const livecopyMd = await readFile('./regional-text.md', 'utf8');
  const langstoreMdast = await getMdastFromMd(langstoreMd);
  const livecopyMdast = await getMdastFromMd(livecopyMd);
  const livecopyProcessedMdast = await getProcessedMdast(livecopyMdast);
  const langstoreNowProcessedMdast = await getProcessedMdast(langstoreMdast);
  const mergedMdast = await getMergedMdast(langstoreNowProcessedMdast, livecopyProcessedMdast);
  const docx = await mdast2docx(mergedMdast, mdast2docxOptions);
  await writeFile('merged.docx', docx);
  process.exit(0);
}
