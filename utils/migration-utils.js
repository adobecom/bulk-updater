...
export async function updateSave(mdast, sourceDocxFile, outputDocxFile) {
  ...
  if (!sourceDocxFile)
    return { status: STATUS_FAILED, message: `No source docx file` };
  if (!outputDocxFile)
    return { status: STATUS_FAILED, message: `No output docx file` };
  ...
  try {
    ...
    if (saved) {
      ...
      return {
        ...
        message: `Error saving ${outputDocxFile}`,
      };
    }
  } catch (e) {
    return {
      status: STATUS_FAILED,
      message: `Error updating and saving ${sourceDocxFile} ${e.message}`,
    };
  }
}
...
const migrateBlock = async (mdast, block, migrate, entry) => {
  ...
  try {
    ...
    const blockReports = Array.isArray(migrationReport)
      ? migrationReport
      : [migrationReport];
    ...
    blockReports.forEach((blockReport) => {
      ...
      if (blockReport.status !== STATUS_SUCCESS) {
        baseReport.status =
          blockReport.status === STATUS_ERROR
            ? STATUS_FAILED
            : blockReport.status;
...
export const migrateBlocks = async (mdast, migrationMap, entry) => {
  ...
  const pageReport = {
    status: { entry, status: STATUS_SUCCESS, blocks },
    migrations: [],
  };
...
export const migratePath = async (mdast, entry, path, migrate) => {
  ...
  const pageReport = {
    status: { entry, status: STATUS_SUCCESS, path },
    migrations: [],
  };

  try {
    const migrationReport = await migrate(mdast, entry);
    const pathReports = Array.isArray(migrationReport)
      ? migrationReport
      : [migrationReport];
    ...
    pathReports.forEach((pathReport) => {
      ...
      if (pathReport.status !== STATUS_SUCCESS) {
        pageReport.status.status =
          pathReport.status === STATUS_ERROR
            ? STATUS_FAILED
            : pathReport.status;
