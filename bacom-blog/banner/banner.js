...
export const BANNERS_HOSTNAMES = ['business.adobe.com', 'main--business-website--adobe.hlx.page'];
...
export default function convertBanner(mdast) {
    ...
    return bannerBlocks.map((block) => {
        ...
        try {
            ...
            return { status: STATUS_ERROR, message: 'Invalid URL', bannerLink: link.url };
        }
        const { hostname, pathname, search, hash, href } = parsedUrl;

        if (!BANNERS_HOSTNAMES.includes(hostname)) {
            return { status: STATUS_ERROR, message: `${hostname} does not contain "${BANNERS_HOSTNAMES.join('" or "')}"`, bannerLink: link.url };
        } else if (!pathname.includes(BANNERS_PATH)) {
            return { status: STATUS_ERROR, message: `${pathname} does not contain "${BANNERS_PATH}"`, bannerLink: link.url };
        }
        ...
        const report = { bannerEntry: pathname, bannerLink: href, fragmentLink: fragmentUrl };
        if (block.type === 'paragraph' && block.children[0] === link) {
            return { status: STATUS_SUCCESS, message: 'Banner converted to fragment link', ...report };
        } else {
            return { status: STATUS_WARNING, message: 'Block migration might not have been successful', ...report };
        }
    });
}
