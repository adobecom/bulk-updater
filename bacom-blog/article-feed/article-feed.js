...
export default function updateArticleFeed(mdast, entry) {
    ...
    if (!feedURL) {
        return { status: STATUS_SKIPPED, message: 'Entry does not contain AU or UK path.' };
    }
    ...
    if (header?.children?.length === 2) moveNode(header.children[1], header, body);
    body.children.push(getFeedRow(feedURL));
    
    return { status: STATUS_SUCCESS, message: 'Custom feed link added to Article Feed' };
}